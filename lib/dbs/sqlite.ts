...
export async function hasStoreUser(storeHash: string, userId: string) {
    ...
    const row = db.prepare('SELECT 1 FROM store_users WHERE user_id = ? AND store_hash = ?').get(userId, storeHash);
...
export async function getStoreToken(storeHash: string) {
    ...
    const row = db.prepare('SELECT access_token FROM stores WHERE store_hash = ?').get(storeHash) as { access_token: string } | undefined;
...
export async function deleteUser({ context, user, sub }: SessionProps) {
    ...
    db.prepare('DELETE FROM store_users WHERE user_id = ? AND store_hash = ?').run(String(user?.id), storeHash);
}
